...
def print_text_diagram():
    print(heading("WORKFLOW DIAGRAM"))
...
def print_comparison_note(bfs_order, dfs_order, alternate_bfs_order):
    print(note_text("TRAVERSAL COMPARISON"))
    print(f"{bfs_text('BFS from ' + START_NODE + ':')} {bfs_order}")
    print(f"{dfs_text('DFS from ' + START_NODE + ':')} {dfs_order}")
    print(f"{bfs_text('BFS from ' + ALTERNATE_START_NODE + ':')} {alternate_bfs_order}")
    print()
    print(
        note_text(
...
def print_model_limit_note():
    print(note_text("MODEL LIMIT NOTE"))
    print(
        note_text(
...
def main():
    ...
    print(
        heading(
            "LAB 06 OPTIONAL COLORIZED SUCCESS VERSION - GRAPH TRAVERSAL AND MODELING"
        )
    )
    print(f"System: {note_text('help desk escalation workflow')}")
    print()
    print_text_diagram()
